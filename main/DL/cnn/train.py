...
class CNN_Model(nn.Module):
    def __init__(self, input_size, num_classes=8):
        ...
        # 将输入重塑为适合CNN的格式 (batch_size, 1, input_size)
        ...
        # 计算卷积层输出大小
        conv_output_size = input_size // (2 ** 3)  # 经过3次池化
        
        self.fc1 = nn.Linear(128 * conv_output_size, 256)
        self.fc2 = nn.Linear(256, num_classes)

    def forward(self, x):
        # 重塑输入为 (batch_size, 1, input_size)
        x = x.unsqueeze(1)
    ...
    for epoch in range(num_epochs):
        ...
        if valid_iter is not None:
            ...
            if valid_accuracy > best_accuracy:
                ...
                best_epoch = epoch
                torch.save(net.state_dict(), r"model/best_model_cnn.pth")
...
def plot_confusion_matrix(y_true, y_pred, class_names):
    ...
    plt.tight_layout()
    plt.savefig(r'main/DL/draw/confusion_matrix_cnn.png', dpi=300, bbox_inches='tight')
    plt.close()

def main_cnn(num_epochs=1, lr=5e-4, wd=1e-5, batch_size=256):
