...
def preprocess_data(train_df):
    ...
    # 检查标准差是否为 0 或 NaN，并替换为 1
    stds = stds.replace(0, 1).fillna(1)

    # 标准化
    X_train = (X_train_df - means) / stds
    X_train = X_train.fillna(0)  # 填充可能的 NaN 值
...
class LSTM_Model(nn.Module):
    ...
    def forward(self, x):
        ...
        # LSTM 需要 (batch_size, seq_length, input_size)，这里假设每个样本作为一个时间步
        x = x.unsqueeze(1)  # (batch_size, 1, input_size)

        # LSTM 部分
        lstm_out, (hn, cn) = self.lstm(x)  # lstm_out: (batch_size, 1, hidden_size)
    ...
    for epoch in range(num_epochs):
        ...
        if valid_iter is not None:
            ...
            if valid_accuracy > best_accuracy:
                ...
                #     os.makedirs('./model')
                torch.save(net.state_dict(), r"model/best_model_lstm.pth")
...
def plot_confusion_matrix(y_true, y_pred, class_names):
    ...
    plt.tight_layout()
    plt.savefig(r'main/DL/draw/confusion_matrix_lstm.png', dpi=300, bbox_inches='tight')
    plt.close()




def main(num_epochs=1, lr=5e-4, wd=1e-5, batch_size=256):
